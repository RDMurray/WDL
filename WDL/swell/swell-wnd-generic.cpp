...
static void swell_edit_apply_selection_state(HWND hwnd, __SWELL_editControlState *es, int anchor, int focus, bool notify_atspi)
{
  if (!hwnd || !es) return;
  ...
  InvalidateRect(hwnd,NULL,FALSE);
  if (notify_atspi) swell_atspi_window_changed(hwnd);
}

bool swell_edit_control_get_atspi_text_state(HWND hwnd, int *cursor_pos, int *sel_start, int *sel_end, int *scroll_x)
{
...
bool swell_edit_control_set_atspi_selection(HWND hwnd, int anchor, int focus)
{
  if (!swell_is_edit_hwnd(hwnd) || !hwnd->m_private_data) return false;
